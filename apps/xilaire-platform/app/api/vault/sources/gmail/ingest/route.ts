...
function isGmailSource(source: VaultSourceRow | null) {
  if (!source) return false;

  const sourceType = (source.source_type ?? "").toLowerCase();
  const provider = (source.provider ?? "").toLowerCase();

  return sourceType === "gmail_api" || provider === "google_workspace";
}
...
async function upsertGmailMailbox(params: {
  orgId: string;
  sourceId: string;
...
async function queueInitialSyncJob(params: {
  ...
  return normalizeString((data as { id?: unknown } | null)?.id);
}
...
async function processDiscoverMailboxesJob(params: {
  ...
  if (!selectedMailboxes.length) {
    return {
      ...
      message:
        "No selected Gmail mailboxes were present. Mailbox discovery is ready, but full Google Workspace user discovery is not implemented yet.",
  ...
  if (params.dryRun) {
    return {
      ...
      sourceId: params.source.id,
      mailboxId: params.job.mailbox_id,
      message: "Dry run completed. Selected Gmail mailboxes were validated.",
  ...
  for (const mailboxAddress of selectedMailboxes) {
    const mailbox = await upsertGmailMailbox({
      orgId: params.orgId,
      sourceId: params.source.id,
      mailboxAddress,
    });
  ...
  return {
    ...
    mailboxId: params.job.mailbox_id,
    message:
      "Selected Gmail mailboxes were discovered and initial sync jobs were queued.",
    discoveredMailboxes,
    queuedJobs,
  };
}
...
async function processMailboxSyncJob(params: {
  ...
  if (!mailboxAddress) {
    return {
      ...
      mailboxId: params.mailbox.id,
      message:
        "Mailbox sync failed because mailbox address could not be resolved.",
  ...
  if (params.dryRun) {
    return {
      ...
      sourceId: params.source.id,
      mailboxId: params.mailbox.id,
      message: `Dry run completed for Gmail mailbox ${mailboxAddress}. Auth method: ${authMethod}.`,
      processedMessages: 0,
    };
  }
  ...
  /*
    Gmail API implementation goes here next.

    Next implementation layer:
    - Build a Gmail API client from vault_sources.connection_config/config.
    - For service account domain-wide delegation, impersonate mailboxAddress.
    - Call users.messages.list for selected labels / query scope.
    - Call users.messages.get with format=raw or full.
    - Normalize headers and body into vault_messages.
    - Create vault_message_occurrences.
    - Create vault_message_attachments rows and storage objects if needed.
    - Store Gmail historyId or next page cursor in vault_source_mailboxes.sync_cursor.
  ...
  */
  ...
  return {
    ...
    sourceId: params.source.id,
    mailboxId: params.mailbox.id,
    message: `Gmail mailbox ${mailboxAddress} sync framework completed. Gmail API fetch layer is ready to be plugged in next.`,
    processedMessages: 0,
  };
}
...
async function processJob(params: {
  ...
  try {
    if (!params.job.source_id) {
      throw new Error("Gmail ingestion job is missing source_id.");
    }
    ...
    if (!isGmailSource(source)) {
      throw new Error(
        "Ingestion job source is not a Gmail / Google Workspace source."
      );
    }
    ...
    if (params.job.job_type === "discover_mailboxes") {
      ...
      if (!mailbox) {
        throw new Error("Mailbox for Gmail ingestion job was not found.");
      }
      ...
      result = {
        ...
        sourceId: params.job.source_id,
        mailboxId: params.job.mailbox_id,
        message: `Unsupported Gmail ingestion job type: ${params.job.job_type}`,
      };
    }
    ...
    const message =
      error instanceof Error ? error.message : "Unknown Gmail ingestion error.";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const body = (await request.json().catch(() => ({}))) as GmailIngestRequest;
    ...
    await writeUnifiedVaultAccessAuditLog({
      access,
      request,
      action: "vault.source.gmail.ingest",
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Unable to process Gmail ingestion jobs.";
