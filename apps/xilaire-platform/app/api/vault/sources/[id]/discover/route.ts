  ...
  try {
    ...
    if (jobError) {
      await writeUnifiedVaultAccessAuditLog({
        access,
        action: "vault.source.discover.failed",
      ...
      return jsonError("Failed to queue mailbox discovery job.", 500, {
    ...
    if (updateError) {
      return jsonError("Discovery job was queued, but source status update failed.", 500, {
    ...
    return jsonOk(
      {
        ...
        sourceId,
        job,
        message: "Mailbox discovery job queued.",
      },
      201
    );
  } catch (error) {
    const message =
      error instanceof Error
        ? error.message
        : "Unknown error queueing mailbox discovery.";

    return jsonError("Failed to queue mailbox discovery.", 500, {
      details: message,
    });
  }
}