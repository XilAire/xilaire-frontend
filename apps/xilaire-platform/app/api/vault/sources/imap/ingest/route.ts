...
function isImapSource(source: VaultSourceRow | null) {
  if (!source) return false;

  const sourceType = (source.source_type ?? "").toLowerCase();
  const provider = (source.provider ?? "").toLowerCase();

  return sourceType === "imap" || provider === "api";
}
...
async function upsertImapMailbox(params: {
  orgId: string;
  sourceId: string;
...
async function processDiscoverMailboxesJob(params: {
  ...
  const username =
    normalizeString(sourceConfig.username) ?? getPayloadString(payload, "username");
  ...
  if (!selectedMailboxes.length) {
    return {
      ...
      message:
        "No selected IMAP mailboxes were present. Add selectedMailboxes or username in the source configuration.",
  ...
  if (params.dryRun) {
    return {
      ...
      sourceId: params.source.id,
      mailboxId: params.job.mailbox_id,
      message: "Dry run completed. Selected IMAP mailboxes were validated.",
  ...
  for (const mailboxAddress of selectedMailboxes) {
    const mailbox = await upsertImapMailbox({
      orgId: params.orgId,
      sourceId: params.source.id,
      mailboxAddress,
    });
  ...
  return {
    ...
    mailboxId: params.job.mailbox_id,
    message:
      "Selected IMAP mailboxes were discovered and initial sync jobs were queued.",
    discoveredMailboxes,
    queuedJobs,
  };
}
...
async function processMailboxSyncJob(params: {
  ...
  const host =
    normalizeString(sourceConfig.host) ?? getPayloadString(payload, "host");

  const username =
    normalizeString(sourceConfig.username) ??
    getPayloadString(payload, "username");
  ...
  if (!host) {
    return {
      ...
      sourceId: params.source.id,
      mailboxId: params.mailbox.id,
      message: "IMAP sync failed because host could not be resolved.",
      error: "Missing IMAP host.",
    };
  }
  ...
  if (!username) {
    return {
      ...
      sourceId: params.source.id,
      mailboxId: params.mailbox.id,
      message: "IMAP sync failed because username could not be resolved.",
  ...
  if (!mailboxAddress) {
    return {
      ...
      sourceId: params.source.id,
      mailboxId: params.mailbox.id,
      message: "IMAP sync failed because mailbox address could not be resolved.",
  ...
  if (params.dryRun) {
    return {
      ...
      sourceId: params.source.id,
      mailboxId: params.mailbox.id,
      message: `Dry run completed for IMAP mailbox ${mailboxAddress} on host ${host}. Auth method: ${authMethod}.`,
      processedMessages: 0,
    };
  }
  ...
  /*
    IMAP implementation goes here next.

    Next implementation layer:
    - Add an IMAP client library, such as imapflow.
    - Resolve password from source.connection_config.password or passwordEnvVar.
    - Connect to host/port/secure.
    - Select folders from folderScope.
    - Fetch messages by UID range or since date.
    - Parse RFC822/MIME with a mail parser.
    - Normalize headers/body into vault_messages.
    - Create vault_message_occurrences.
    - Create vault_message_attachments rows and storage objects if needed.
    - Store highest UID / UIDVALIDITY / folder cursor in vault_source_mailboxes.sync_cursor.

    This worker currently marks the sync framework as completed without writing
    placeholder messages, because we do not want fake evidence in Vault.
  */
  ...
  return {
    ...
    sourceId: params.source.id,
    mailboxId: params.mailbox.id,
    message: `IMAP mailbox ${mailboxAddress} sync framework completed. IMAP fetch/parser layer is ready to be plugged in next.`,
    processedMessages: 0,
  };
}
...
async function processJob(params: {
  ...
  try {
    if (!params.job.source_id) {
      throw new Error("IMAP ingestion job is missing source_id.");
    }
    ...
    if (!isImapSource(source)) {
      throw new Error("Ingestion job source is not an IMAP source.");
    }
    ...
    if (params.job.job_type === "discover_mailboxes") {
      ...
      if (!mailbox) {
        throw new Error("Mailbox for IMAP ingestion job was not found.");
      }
      ...
      result = {
        ...
        sourceId: params.job.source_id,
        mailboxId: params.job.mailbox_id,
        message: `Unsupported IMAP ingestion job type: ${params.job.job_type}`,
      };
    }
    ...
    const message =
      error instanceof Error ? error.message : "Unknown IMAP ingestion error.";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const body = (await request.json().catch(() => ({}))) as ImapIngestRequest;
    ...
    await writeUnifiedVaultAccessAuditLog({
      access,
      request,
      action: "vault.source.imap.ingest",
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Unable to process IMAP ingestion jobs.";
