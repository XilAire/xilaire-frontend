...
function isGraphSource(source: VaultSourceRow | null) {
  ...
  return (
    ...
    (sourceType === "graph" && provider === "microsoft_365")
  );
}
  ...
  return Array.from(
    new Set(
      input
        ...
        .filter(Boolean)
        .map((item) => {
          if (item === "mail" || item === "mailboxes") return "exchange";
          if (item === "users" || item === "groups") return "users_groups";
          if (item === "audit") return "audit_logs";
          return item;
        })
...
function getWorkloadsFromSource(source: VaultSourceRow) {
  const scopeConfig = getSourceScopeConfig(source);
  const workloads = normalizeWorkloads(scopeConfig.workloads);

  if (workloads.length) return workloads;

  const fallbackWorkloads = [
    scopeConfig.includeTeams === true ? "teams" : null,
    scopeConfig.includeSharePoint === true ? "sharepoint" : null,
    scopeConfig.includeOneDrive === true ? "onedrive" : null,
    scopeConfig.includeUsersGroups === true ? "users_groups" : null,
    scopeConfig.includeAuditLogs === true ? "audit_logs" : null,
    "exchange",
  ].filter((item): item is string => Boolean(item));

  return Array.from(new Set(fallbackWorkloads));
}
...
async function upsertGraphMailbox(params: {
  orgId: string;
  sourceId: string;
...
async function processDiscoverMailboxesJob(params: {
  ...
  const workloads = Array.from(
    new Set([
      ...normalizeWorkloads(payload.workloads),
      ...getWorkloadsFromSource(params.source),
      "exchange",
    ])
  );
  ...
  if (!selectedMailboxes.length) {
    return {
      ...
      message:
        "No selected Graph mailboxes were present. Full tenant mailbox discovery is ready for the Graph API layer.",
  ...
  if (params.dryRun) {
    return {
      ...
      sourceId: params.source.id,
      mailboxId: params.job.mailbox_id,
      message: "Dry run completed. Selected Graph mailboxes were validated.",
  ...
  for (const mailboxAddress of selectedMailboxes) {
    const mailbox = await upsertGraphMailbox({
      orgId: params.orgId,
      sourceId: params.source.id,
      mailboxAddress,
      workloads,
  ...
  return {
    ...
    mailboxId: params.job.mailbox_id,
    message:
      "Selected Graph mailboxes were discovered and initial sync jobs were queued.",
    discoveredMailboxes,
    queuedJobs,
  };
}

async function processDiscoverWorkloadsJob(params: {
  orgId: string;
  job: VaultIngestionJobRow;
  source: VaultSourceRow;
  dryRun: boolean;
  ...
  const normalizedWorkloads = workloads.length ? workloads : ["exchange"];
  ...
  return {
    ...
    message: params.dryRun
      ? "Dry run completed. Microsoft Graph workloads were validated."
      : "Microsoft Graph workload discovery framework completed. Graph workload API fetch layer is ready to be plugged in next.",
    discoveredWorkloads: normalizedWorkloads,
    queuedJobs: 0,
  };
}
...
async function processMailboxSyncJob(params: {
  ...
  const payload = params.job.payload ?? {};
  const tenantId =
    normalizeString(sourceConfig.tenantId) ?? getPayloadString(payload, "tenantId");
  const clientId =
    normalizeString(sourceConfig.clientId) ?? getPayloadString(payload, "clientId");
  const authMethod =
  ...
  if (!tenantId) {
    return {
      ...
      sourceId: params.source.id,
      mailboxId: params.mailbox.id,
      message: "Graph sync failed because tenantId could not be resolved.",
      error: "Missing Microsoft Graph tenantId.",
    };
  }
  ...
  if (!clientId) {
    return {
      ...
      sourceId: params.source.id,
      mailboxId: params.mailbox.id,
      message: "Graph sync failed because clientId could not be resolved.",
      error: "Missing Microsoft Graph clientId.",
    };
  }
  ...
  if (!mailboxAddress) {
    return {
      ...
      mailboxId: params.mailbox.id,
      message:
        "Graph sync failed because mailbox address could not be resolved.",
  ...
  if (params.dryRun) {
    return {
      ...
      sourceId: params.source.id,
      mailboxId: params.mailbox.id,
      message: `Dry run completed for Graph mailbox ${mailboxAddress}. Auth method: ${authMethod}. Workloads: ${workloads.join(", ")}.`,
      processedMessages: 0,
    };
  }
  ...
  /*
    Microsoft Graph API implementation goes here next.

    Next implementation layer:
    - Build an app-only Microsoft Graph client from vault_sources.connection_config/config.
    - Resolve clientSecret from clientSecret or clientSecretEnvVar.
    - Acquire token from /{tenantId}/oauth2/v2.0/token.
    - For Exchange: call /users/{mailbox}/messages with selected fields and paging.
    - For Teams: call Teams/channel/chat APIs when permissions are configured.
    - For SharePoint/OneDrive: call drive/list item APIs and normalize files.
    - For users/groups: call /users and /groups for directory snapshots.
    - For audit logs: call the appropriate audit endpoint once enabled.
    - Normalize Exchange messages into vault_messages.
    - Create vault_message_occurrences.
    - Create vault_message_attachments rows and storage objects if needed.
    - Store Graph deltaLink/nextLink in vault_source_mailboxes.sync_cursor.
  ...
  */
  ...
  return {
    ...
    sourceId: params.source.id,
    mailboxId: params.mailbox.id,
    message: `Microsoft Graph mailbox ${mailboxAddress} sync framework completed. Graph API fetch/parser layer is ready to be plugged in next.`,
    processedMessages: 0,
  };
}
...
async function processJob(params: {
  ...
  try {
    if (!params.job.source_id) {
      throw new Error("Microsoft Graph ingestion job is missing source_id.");
    }
    ...
    if (!isGraphSource(source)) {
      throw new Error(
        "Ingestion job source is not a Microsoft Graph API source."
      );
    }
    ...
    if (params.job.job_type === "discover_mailboxes") {
    ...
    } else if (params.job.job_type === "discover_workloads") {
      result = await processDiscoverWorkloadsJob({
        orgId: params.orgId,
        job: params.job,
        source,
        dryRun: params.dryRun,
      ...
      if (!mailbox) {
        throw new Error("Mailbox for Graph ingestion job was not found.");
      }
      ...
      result = {
        ...
        sourceId: params.job.source_id,
        mailboxId: params.job.mailbox_id,
        message: `Unsupported Graph ingestion job type: ${params.job.job_type}`,
      };
    }
    ...
    const message =
      error instanceof Error ? error.message : "Unknown Graph ingestion error.";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const body = (await request.json().catch(() => ({}))) as GraphIngestRequest;
    ...
    await writeUnifiedVaultAccessAuditLog({
      access,
      request,
      action: "vault.source.graph.ingest",
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Unable to process Microsoft Graph ingestion jobs.";
