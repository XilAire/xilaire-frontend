...
function buildDisplayName(payload: EmlImportRequest) {
  if (payload.name?.trim()) {
    return payload.name.trim();
  }

  if (payload.mailboxAddress?.trim() && payload.fileName?.trim()) {
    return `EML Import - ${payload.mailboxAddress.trim()} - ${payload.fileName.trim()}`;
  }

  if (payload.fileName?.trim()) {
    return `EML Import - ${payload.fileName.trim()}`;
  }

  return "EML Import Source";
}
...
async function findExistingPendingMailboxJob(params: {
  ...
  sourceId: string;
  mailboxId: string;
  jobType: "eml_parse" | "eml_import" | "initial_sync";
}) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!orgId) {
      return jsonError(
        "Unable to resolve organization context for EML import.",
        403
      );
    }

    const body = (await request.json()) as EmlImportRequest;
    ...
    const contentType = normalizeString(body.contentType) ?? "message/rfc822";
    ...
    if (!validateRequiredString(mailboxAddress)) {
      return jsonError("mailboxAddress is required for EML import.", 400);
    }

    if (!validateRequiredString(fileName)) {
      return jsonError("fileName is required for EML import.", 400);
    }

    if (!validateRequiredString(storageBucket)) {
      return jsonError("storageBucket is required for EML import.", 400);
    }

    if (!validateRequiredString(storagePath)) {
      return jsonError("storagePath is required for EML import.", 400);
    }
    ...
    if ((existingSource as unknown as { id?: string } | null)?.id) {
      ...
      if (updateSourceError || !updatedSource) {
        return jsonError(
          updateSourceError?.message || "Unable to update EML import source.",
          500
        );
      }
      ...
      if (insertSourceError || !insertedSource) {
        return jsonError(
          insertSourceError?.message || "Unable to create EML import source.",
          500
        );
      }
    ...
    if (mailboxUpsertError || !upsertedMailbox) {
      return jsonError(
        mailboxUpsertError?.message || "Unable to create EML mailbox target.",
        500
      );
    }
    ...
    await writeUnifiedVaultAccessAuditLog({
      access: auth,
      request,
      action: "vault.import.eml.create",
    ...
    const message =
      error instanceof Error ? error.message : "Unable to create EML import.";
