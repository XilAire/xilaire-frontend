...
function buildFormErrorRedirect(message: string) {
  redirect(`/vault/imports/pst?error=${encodeURIComponent(message)}`);
}

async function createPstImport(formData: FormData) {
  "use server";
  ...
  const contentType =
    normalizeString(formData.get("contentType")) ?? "application/vnd.ms-outlook";
  ...
  const includeDeletedItems = formData.get("includeDeletedItems") !== "false";
  const includeRecoverableItems =
    formData.get("includeRecoverableItems") === "true";
  ...
  if (!mailboxAddress) {
    buildFormErrorRedirect("Mailbox address is required for PST import.");
  }

  if (!fileName) {
    buildFormErrorRedirect("PST file name is required.");
  }
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000"}/api/vault/imports/pst`,
    {
      method: "POST",
      headers: {
  ...
  if (!response.ok) {
    ...
    const message =
      ...
      "Failed to create PST import.";

    buildFormErrorRedirect(message);
  }

  revalidatePath("/vault/imports/pst");
  revalidatePath("/vault/sources");
  revalidatePath("/vault/ingestion");

  redirect("/vault/imports/pst?success=pst-import-created");
}

export default async function VaultPstImportPage({
  searchParams,
}: PageContext) {
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-6">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-400">
                Register uploaded PST files for Vault ingestion. This creates a
                manual upload source, links the PST to a mailbox target, and
                queues PST parse/import jobs for processing.
              </p>
            </div>
        ...
        {(error || success) && (
          ...
          >
            ...
            {success === "pst-import-created" ? (
              <p className="text-sm">
                PST import source created successfully and ingestion jobs were
                queued.
              </p>
            ) : null}
          </section>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/70">
            <div className="border-b border-zinc-800 px-5 py-4">
              ...
              <p className="mt-1 text-sm text-zinc-500">
                Use this after the PST has already been uploaded into your
                storage bucket.
              </p>
            </div>

            <form action={createPstImport} className="grid gap-5 p-5">
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <input
                    id="name"
                    name="name"
                    placeholder="PST Import - John Smith"
                ...
                <div>
                  ...
                  <input
                    id="sourceKey"
                    name="sourceKey"
                    placeholder="pst-jsmith-archive"
                    className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 outline-none placeholder:text-zinc-600 focus:border-blue-500/60"
                  />
                </div>
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <input
                      id="fileName"
                      name="fileName"
                      placeholder="john-smith-archive.pst"
                      required
                      className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 outline-none placeholder:text-zinc-600 focus:border-blue-500/60"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      id="storagePath"
                      name="storagePath"
                      placeholder="org-id/pst/john-smith-archive.pst"
                      required
                      className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 outline-none placeholder:text-zinc-600 focus:border-blue-500/60"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      id="contentType"
                      name="contentType"
                      defaultValue="application/vnd.ms-outlook"
                      className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 outline-none placeholder:text-zinc-600 focus:border-blue-500/60"
                    />
                  </div>
            ...
            <section className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-5">
              ...
              <div className="mt-4 grid gap-3 text-sm text-zinc-400">
                <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
                  <p className="font-medium text-zinc-200">1. Upload PST</p>
                  <p className="mt-1">
                    Upload the PST into your Vault import storage bucket.
                  </p>
                </div>

                <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
                  <p className="font-medium text-zinc-200">2. Register import</p>
                  <p className="mt-1">
                ...
                <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
                  ...
                  <p className="mt-1">
                    Vault queues pst_parse and pst_import jobs.
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
                  ...
                  <p className="mt-1">
                    The ingestion worker parses the PST and imports normalized
