...
export async function closeExecution({
  ...
  /* -------------------------------------------------
     ...
     entry_cost, exit_value, pnl, and pnl_pct are writable columns.
     Persist the execution engine calculations for debit, credit,
     multi-leg, ratio, and partial-close strategies.
  ------------------------------------------------- */
  ...
  /* -------------------------------------------------
     PARTIAL-CLOSE DISCORD ALERT
  ------------------------------------------------- */
  let partialCloseAlertSent: boolean | null = null;

  if (nextExecutionStatus === "PARTIAL") {
    try {
      partialCloseAlertSent =
        await sendPartialCloseSignalAlert({
          signalId: execution.signal_id,
          closedContracts,
          totalContracts: Math.max(
            toNumber(execution.contracts),
            0,
          ),
          remainingContracts,
          ...
          realizedPnl,
          realizedReturnPct,
          legCloses: isMultiLegClose
            ? normalizedLegCloses.map((legClose) => {
                const legPerformance =
                  multiLegPerformance?.legPerformance.find(
                    (leg) =>
                      leg.legId === legClose.signalOptionLegId,
                  );
                ...
                return {
                  ...
                  realizedPnl:
                    legPerformance
                      ? roundMoney(
                          legPerformance.realizedPnl * 100,
                        )
                      : null,
                };
              })
      ...
      if (!partialCloseAlertSent) {
        console.warn(
          "closeExecution: partial-close Discord alert was not sent",
          {
            executionId,
            signalId: execution.signal_id,
            closedContracts,
            totalContracts: execution.contracts,
            remainingContracts,
          },
        );
      }
    } catch (discordError) {
      partialCloseAlertSent = false;

      console.error(
        "closeExecution: Discord partial-close alert failed",
        {
          executionId,
          signalId: execution.signal_id,
          error: discordError,
