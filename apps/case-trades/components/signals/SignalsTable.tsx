...
export default function SignalsTable({
  ...
  function handleUpdateStatus(id: string, nextStatus: SignalDisplayStatus) {
    ...
    if (nextStatus === "Closed" || nextStatus === "Expired") {
      ...
      router.push(withOrgQuery(`/dashboard/signals/${id}`, selectedOrganizationSlug));
      return;
    }
      ...
      normalizeSignals(prev).map((signal) =>
        signal.id === id
          ? {
              ...
              watching: nextWatching,
              watched:
                shouldClearWatchingForDisplayStatus(nextStatus) && signal.watching
                  ? true
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <select
              ...
              className="rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-sm text-slate-200 outline-none hover:bg-slate-900"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-sm text-slate-200 hover:bg-slate-900"
            >
              <ArrowUpDown className="h-4 w-4" />
              Date {sortKey === "created_at" ? `(${sortDir})` : ""}
            </button>
