...
type SignalStatus = "Active" | "Triggered" | "Expired";
...
function buildSignalsUrl({
  ...
  return `/dashboard/master-admin/signals?${params.toString()}`;
}
...
export default async function SignalsPage({
  ...
  if (accessibleOrganizations.length === 0) {
    ...
    accessibleOrganizations = userOrganizations as OrganizationOption[];
  }
  ...
  if (statusFilter === "active_recent") {
    if (rangeStart) {
      query = query.or(`status.eq.Active,created_at.gte.${rangeStart}`);
    } else {
      query = query.eq("status", "Active");
    }
  } else {
    if (rangeStart && statusFilter !== "active") {
  ...
  const watchedSignals = signals.filter(
    (signal) => signal.watched || signal.watching
  ).length;
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        ...
        <Link
          href={`/dashboard/master-admin/signals/create?org=${selectedOrganization.slug}`}
        ...
        <div className="mb-6 space-y-5">
          <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
            <div>
              ...
              <p className="text-sm text-slate-400">
                Viewing signals from {selectedOrganization.name}. Master admins
                can update status, watch signals, and delete signal records.
              </p>
            </div>
