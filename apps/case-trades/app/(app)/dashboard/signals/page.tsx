...
async function deleteSignalAction(formData: FormData) {
  ...
  const selectedOrganization = accessibleOrganizations.find(
    (accessibleOrg) => accessibleOrg.id === organizationId
  );

  const canAccessOrganization = Boolean(selectedOrganization);
  ...
  const canDeleteSignals = canManageOrganizationSignals({
    profile,
...
export default async function SignalsPage({
  ...
  const isMasterAdmin = isProfileMasterAdmin(profile);
  ...
  if (accessibleOrganizations.length === 0) {
    redirect("/dashboard/billing?product=signals&reason=subscribe");
  }
  ...
  const canManageSelectedOrganization = canManageOrganizationSignals({
    profile,
    organization: selectedOrganization,
  });
  ...
  const watchedSignals = signals.filter((signal) => {
    const displayStatus = getSignalDisplayStatus(signal);

    return displayStatus === "Watching" || signal.watched || signal.watching;
  }).length;
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        ...
        {(isMasterAdmin || canManageSelectedOrganization) && (
          <Link
        ...
        <div className="mb-6 space-y-5">
          <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
            ...
            <p className="text-xs text-slate-500">
              Execution quantities and P/L are derived from fills and enforced
              centrally.
            </p>
          </div>
