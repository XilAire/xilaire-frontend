...
export async function POST() {
  try {
    ...
    /*
     * Billing belongs to the active workspace.
     *
     * An invited member can receive the paid entitlements of the
     * workspace they joined, but that does not make them the billing
     * owner of that workspace.
     *
     * Only the workspace owner may cancel the workspace subscription.
     */
    ...
    /*
     * Find the subscription for the active workspace rather than simply
     * finding the newest subscription owned by the authenticated user.
     *
     * This allows one user to own multiple independent workspaces and
     * prevents cancellation of the wrong Stripe subscription.
     */
    const subscription =
    ...
    /*
     * The subscription's billing owner should always be the workspace
     * owner. Protect against malformed or legacy records before making
     * any Stripe mutation.
     */
    if (
      subscription.userId !==
      ...
      console.error(
        "[CASE Budget Stripe Subscription Cancel] Workspace subscription billing owner mismatch.",
        {
          workspaceId:
            billingAuthorization.workspaceId,
...
async function authorizeWorkspaceBilling({
  ...
  if (
    ...
    console.error(
      "[CASE Budget Stripe Subscription Cancel] Workspace lookup failed.",
      {
        userId:
          normalizedUserId,
  ...
  if (
    ...
    console.error(
      "[CASE Budget Stripe Subscription Cancel] Workspace membership lookup failed.",
      {
        userId:
          normalizedUserId,
...
async function findCancelableSubscription({
  ...
  if (
    ...
    console.error(
      "[CASE Budget Stripe Subscription Cancel] Workspace subscription lookup failed.",
      {
        workspaceId:
          normalizedWorkspaceId,
