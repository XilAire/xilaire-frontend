...
export type ArchiveIncomeSourceInput = {
  incomeSourceId:
    string;
...
export type ArchiveIncomeSourceRecord = {
  id:
    string;
...
export type ArchiveCaseBudgetIncomeSourceResult =
  | {
      success:
        true;
  ...
  | {
      ...
      error: {
        code:
          ...
          | "income-source-archive-conflict"
          | "income-source-archive-failed"
...
/**
 * Archives or restores one monthly income source.
 *
 * Production rules:
 *
 ...
 * - Archival is soft; the database row is preserved.
 * - Restoring reactivates the existing historical row.
 * - Household budget-change approval runs before mutation.
 * - If approval is required, the income-source row remains unchanged.
 * - updated_at optimistic concurrency prevents stale changes.
 * - Parent month planned_income / actual_income are recalculated from
 *   canonical ACTIVE income-source rows after a successful mutation.
 * - If aggregate maintenance fails, the archival state is rolled back
 *   where possible.
...
): Promise<ArchiveCaseBudgetIncomeSourceResult> {
  try {
    ...
    if (
      ...
      return failure({
        ...
        message:
          "View-only members cannot archive or restore income sources.",
      });
    }
    ...
    if (
      ...
      console.error(
        "[CASE Budget Budget] Failed to load income source for archival.",
        {
          workspaceId,
          userId,
          incomeSourceId,
    ...
    if (
      ...
      return failure({
        ...
        message:
          "Income sources cannot be archived or restored in a closed budget month.",
      });
    }
     ...
     * Idempotent behavior.
     *
     * If the source is already in the requested archival state, simply
     * return the canonical row without creating another approval request.
     */
    if (
      ...
      if (
        ...
        return failure({
          ...
          message:
            "CASE Budget could not verify the current income-source state.",
        });
      }
    ...
    if (
      ...
      return failure({
        ...
        message:
          "CASE Budget could not verify the current income-source amounts.",
      });
    }
    ...
    const approvalResult =
      await enforceHouseholdApproval({
        ...
        title:
          shouldArchive
            ? `Archive income source: ${existing.name}`
            : `Restore income source: ${existing.name}`,
    ...
    if (
      ...
      console.error(
        "[CASE Budget Budget] Approval enforcement failed during income-source archival.",
        {
          workspaceId,
          userId,
          incomeSourceId,
    ...
    if (
      ...
      console.error(
        "[CASE Budget Budget] Failed to change income-source archival state.",
        {
          workspaceId,
          userId,
          incomeSourceId,
      ...
      return failure({
        ...
        message:
          shouldArchive
            ? "CASE Budget could not archive the income source."
            : "CASE Budget could not restore the income source.",
      });
    }
    ...
    if (
      ...
      return failure({
        ...
        message:
          "This income source changed before the archival update completed. Refresh the budget and try again.",
      });
    }
    ...
    if (
      ...
      await rollbackIncomeSourceArchival({
        workspaceId,
        incomeSourceId,
        existing,
      });
    ...
    if (
      ...
      return failure({
        ...
        message:
          shouldArchive
            ? "CASE Budget archived the income source but could not verify its updated state."
            : "CASE Budget restored the income source but could not verify its updated state.",
      });
    }
    ...
    console.error(
      "[CASE Budget Budget] Unexpected income-source archival error.",
      error,
    );
    ...
    return failure({
      ...
      message:
        "CASE Budget could not change the income-source archival state. Please try again.",
    });
  }
}
...
async function loadWorkspace({
  ...
  if (
    ...
    console.error(
      "[CASE Budget Budget] Failed to load workspace during income-source archival.",
      {
        workspaceId,
        error,
      },
  ...
  if (
    ...
    return {
      ...
      message:
        "Income sources cannot be archived or restored while this workspace is inactive.",
    };
  }
...
async function loadMembership({
  ...
  if (
    ...
    console.error(
      "[CASE Budget Budget] Failed to verify membership during income-source archival.",
      {
        workspaceId,
        userId,
        error,
  ...
  if (
    ...
    return {
      ...
      message:
        "You do not have active access to archive or restore income sources in this workspace.",
    };
  }
...
async function loadBudgetMonthById({
  ...
  if (
    ...
    console.error(
      "[CASE Budget Budget] Failed to load parent month during income-source archival.",
      {
        workspaceId,
        budgetMonthId,
        error,
...
async function rollbackIncomeSourceArchival({
  workspaceId,
  incomeSourceId,
  existing,
}: {
  ...
  if (
    ...
    console.error(
      "[CASE Budget Budget] Failed to roll back income-source archival state after aggregate failure.",
      {
        workspaceId,
        incomeSourceId,
        error,
...
function mapIncomeSourceRecord({
  ...
  budgetMonth:
    CaseBudgetBudgetMonthDatabaseRow;
}): ArchiveIncomeSourceRecord | null {
  const id =
    normalizeOptionalText(
      row.id,
    );
...
function buildApprovalDescription({
  ...
  return `${action} "${name}" in the ${formatMonthLabel(
    monthKey,
  )} budget. This income source has ${formatCurrency(
    plannedAmount,
  )} planned${receivedText}.`;
}
...
function failure({
  ...
  code:
    Extract<
      ArchiveCaseBudgetIncomeSourceResult,
      {
        success:
          false;
      }
