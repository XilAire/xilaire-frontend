...
import {
  deleteInvestmentPerformanceSnapshot as deleteInvestmentPerformanceSnapshotService,
...
export type DeleteInvestmentPerformanceSnapshotResult =
  | {
      success:
        true;
...
/**
 * Deletes one canonical CASE Budget investment performance snapshot from
...
export async function deleteInvestmentPerformanceSnapshot({
  snapshotId,
}: DeleteInvestmentPerformanceSnapshotInput): Promise<DeleteInvestmentPerformanceSnapshotResult> {
  ...
  try {
    ...
    const result =
      await deleteInvestmentPerformanceSnapshotService({
        workspaceId,
    ...
    if (
      ...
      console.error(
        "[CASE Budget Investments] Failed to delete investment performance snapshot.",
        {
          workspaceId,
          userId,
          snapshotId:
    ...
    console.error(
      "[CASE Budget Investments] Unexpected investment-performance snapshot delete error.",
      error,
    );
    ...
    return failure({
      ...
      message:
        "CASE Budget could not delete the investment performance snapshot. Please try again.",
    });
  }
}
...
> {
  ...
  if (
    ...
    console.error(
      "[CASE Budget Investments] Failed to load active workspace while deleting an investment performance snapshot.",
      {
        workspaceId,
        error,
      },
  ...
  if (
    ...
    return {
      ...
      message:
        "Investment performance snapshots cannot be deleted because this workspace is inactive.",
    };
  }
...
> {
  ...
  if (
    ...
    console.error(
      "[CASE Budget Investments] Failed to verify workspace membership while deleting an investment performance snapshot.",
      {
        workspaceId,
        userId,
        error,
  ...
  if (
    ...
    return {
      ...
      message:
        "You do not have active access to delete investment performance snapshots in this workspace.",
    };
  }
  ...
  if (
    ...
    return {
      ...
      message:
        "Viewers cannot delete investment performance snapshots in this workspace.",
    };
  }
...
function failure({
  ...
  code:
    Extract<
      DeleteInvestmentPerformanceSnapshotResult,
      {
        success:
          false;
      }
