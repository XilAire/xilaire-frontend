...
import {
  createInvestmentActivity as createInvestmentActivityService,
...
export type CreateInvestmentActivityResult =
  | {
      success:
        true;
...
/**
 * Creates one canonical CASE Budget investment activity in the
 ...
 * - the parent investment account is validated inside the active workspace.
 * - when holdingId is supplied, the service verifies that the holding belongs
 *   to the selected investment account in the same workspace.
 * - activity values are validated and normalized by the investment service.
...
export async function createActivity(
  activity:
    CreateInvestmentActivityData,
): Promise<CreateInvestmentActivityResult> {
  if (
    !activity ||
    typeof activity !==
      "object" ||
    ...
    return failure({
      ...
      message:
        "Investment activity details are required.",
    });
  }
  ...
  try {
    ...
    const result =
      await createInvestmentActivityService({
        workspaceId,
        activity,
      });
    ...
    if (
      ...
      console.error(
        "[CASE Budget Investments] Failed to create investment activity.",
        {
          workspaceId,
          userId,
          investmentAccountId:
    ...
    console.error(
      "[CASE Budget Investments] Unexpected investment-activity creation error.",
      error,
    );
    ...
    return failure({
      ...
      message:
        "CASE Budget could not create the investment activity. Please try again.",
    });
  }
}
...
> {
  ...
  if (
    ...
    console.error(
      "[CASE Budget Investments] Failed to load active workspace while creating an investment activity.",
      {
        workspaceId,
        error,
      },
  ...
  if (
    ...
    return {
      ...
      message:
        "Investment activity cannot be created because this workspace is inactive.",
    };
  }
...
> {
  ...
  if (
    ...
    console.error(
      "[CASE Budget Investments] Failed to verify workspace membership while creating an investment activity.",
      {
        workspaceId,
        userId,
        error,
  ...
  if (
    ...
    return {
      ...
      message:
        "You do not have active access to create investment activity in this workspace.",
    };
  }
  ...
  if (
    ...
    return {
      ...
      message:
        "Viewers cannot create investment activity in this workspace.",
    };
  }
...
function getFailureCode(
  ...
  if (
    ...
    normalized.includes(
      "selected investment account, holding, or workspace is no longer available",
    )
  ) {
  ...
  if (
    normalized.includes(
      "selected holding does not belong",
    ) ||
    normalized.includes(
...
function failure({
  ...
  code:
    Extract<
      CreateInvestmentActivityResult,
      {
        success:
          false;
      }
