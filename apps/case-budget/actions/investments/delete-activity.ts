...
import {
  deleteInvestmentActivity as deleteInvestmentActivityService,
...
export type DeleteInvestmentActivityResult =
  | {
      success:
        true;
...
/**
 * Deletes one CASE Budget investment activity from the authenticated
 ...
 *
 * The currently confirmed investment_activities schema does not expose
 * archive fields, so this action follows the canonical service behavior
 * and performs a real database delete.
 */
export async function deleteActivity({
  activityId,
}: DeleteInvestmentActivityInput): Promise<DeleteInvestmentActivityResult> {
  ...
  try {
    ...
    const result =
      await deleteInvestmentActivityService({
        workspaceId,
    ...
    if (
      ...
      console.error(
        "[CASE Budget Investments] Failed to delete investment activity.",
        {
          workspaceId,
          userId,
          activityId:
    ...
    console.error(
      "[CASE Budget Investments] Unexpected investment-activity delete error.",
      error,
    );
    ...
    return failure({
      ...
      message:
        "CASE Budget could not delete the investment activity. Please try again.",
    });
  }
}
...
> {
  ...
  if (
    ...
    console.error(
      "[CASE Budget Investments] Failed to load active workspace while deleting an investment activity.",
      {
        workspaceId,
        error,
      },
  ...
  if (
    ...
    return {
      ...
      message:
        "Investment activity cannot be deleted because this workspace is inactive.",
    };
  }
...
> {
  ...
  if (
    ...
    console.error(
      "[CASE Budget Investments] Failed to verify workspace membership while deleting an investment activity.",
      {
        workspaceId,
        userId,
        error,
  ...
  if (
    ...
    return {
      ...
      message:
        "You do not have active access to delete investment activity in this workspace.",
    };
  }
  ...
  if (
    ...
    return {
      ...
      message:
        "Viewers cannot delete investment activity in this workspace.",
    };
  }
...
function failure({
  ...
  code:
    Extract<
      DeleteInvestmentActivityResult,
      {
        success:
          false;
      }
